...
- **Multi-monitor setups (critical for v1 open-source release)**, including mixed laptop Retina + external non-Retina (or any differing resolution/size) displays:
  ...
  - Full coverage is in TESTER's plan at `docs/testing-multi-display.md` (the "Higher → Lower Resolution Move..." + "Steady-State Residence..." sections, plus verification of the ANCHOR-RES-CROSS / full re-render behavior on Chrome moves). See also the detailed mechanism write-up + agent credits (INVESTIGATOR diagnosis, CODER implementation of the four-layer solution including the explicit anchor-driven production path for moves, TESTER validation) in README Recent Fixes and `docs/compatibility-matrix.md`.
