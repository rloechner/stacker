...
**Fix summary (CODER landed, four iterations):**
...
**CODER's new mechanism (Iteration 2):** 
...
**CODER's final mechanism (Iteration 3 — robust re-home guard + smart-initial logic on startTracking + normal sync paths + home screen mismatch guard):**
...
**New trigger + reset behavior (CODER):**
...
- TESTER has run `script/build_and_run.sh --verify` + post-fix `xcodebuild build` successfully (see build logs in session artifacts). Builds clean after all CODER iterations (1 scale, 2 re-home on change, 3 guard + smart-initial on startTracking + sync paths, 4 anchor-driven full re-render on cross-res AX updates). StackerTests target also compiles cleanly.
...
**This plan was authored by the TESTER agent as the expanded checklist referenced from CONTRIBUTING.md. It is intended for use by the developer, QA, and community when validating multi-display fixes or regressions. The steady-state section was added in the final multi-monitor widget reliability push to close the last reported attachment failure mode. The Chrome anchor cross full re-render subsection was added for the anchor-driven complete re-render solution (Iteration 4).**

---
Last updated: 2026-05-19 (TESTER COMPLETE for anchor-driven full re-render — post-CODER Iteration 4: explicit ANCHOR-RES-CROSS detection in syncVisibility + fullAnchorResolutionChangeReset true complete re-render from fresh AX rect as sole input. Confirmed `xcodebuild build` + `script/build_and_run.sh --verify` green (BUILD SUCCEEDED, exit 0; StackerTests target compiles). Added dedicated subsection "Chrome Anchor Window Cross-Resolution Move — Full Re-Render from Fresh Window Coordinates" with trigger description, exact Chrome title-bar drag repro steps (Retina ↔ non-Retina), success criteria (clean snap from new AX coords only, no stale state), and log observation instructions. Updated Fix summary (now four iterations), How to confirm, Edge Cases, Verification, How Testers record, and Sign-Off Criteria for open source (now explicitly requires exercising + passing the new full re-render layer on real mixed-res hardware during Chrome anchor crosses, in addition to the prior steady-state small-monitor bar; three defense layers must all be green). This scenario + the new path must pass for open-source release on actual differing-scale-factor displays. Laptop + one external sufficient for validation.)
