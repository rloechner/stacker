...
**Multi-monitor, Spaces, full-height windows, and widget edge-dragging** have been exercised during normal use across the tested browsers. Different-resolution displays (Retina + non-Retina) are now correctly supported for both widget *rendering* and *positioning*, with a new guarantee of reliable **steady-state attachment even when the widget permanently resides on smaller secondary / lower-resolution monitors**. 

The final robust mechanism (landed via multi-agent collaboration — INVESTIGATOR diagnosed the startTracking + persisted-offset mismatch cases; CODER implemented the complete layered solution including the new explicit anchor-driven path; TESTER authored/expanded the plan in `docs/testing-multi-display.md`) now consists of four explicit defense layers for mixed-resolution Chrome (and other browser) stack moves and steady-state residence:
...
This closes a long-standing class of multi-monitor / multi-resolution attachment bugs for both live moves (via the explicit full re-render path) and steady-state residence on secondary monitors (via the guards). The floating widget is now production-ready for real-world users with mixed laptop + external monitor setups who leave stacks resident on any combination of monitor sizes. Higher→lower, lower→higher, move-then-settle, and "already settled on secondary" cases all pass cleanly on real hardware. See TESTER's full plan (`docs/testing-multi-display.md`), the detailed comments in OverlayRuntime.swift (especially the ANCHOR-RES-CROSS block and `fullAnchorResolutionChangeReset`), and `rehomeIfCurrentResolvedIsBad` for future maintainers. The architecture has four coordinated defense layers with the anchor-driven full re-render as the explicit production solution for Chrome stack moves across mixed-res monitors.
...
- **Multi-resolution display widget rendering + positioning** (Retina + non-Retina mixes): **Resolved and production-validated for v1** — rendering (CODER scale sync via viewDidChangeBackingProperties + recursive contentsScale) + full positioning story now includes the explicit **anchor-driven full re-render** (fresh AX rect backing scale vs. `lastAnchorFrame` detection in `syncVisibility` triggering `fullAnchorResolutionChangeReset` — complete clean-slate re-render from the *new window coordinates* as sole authoritative input, with orderOut, pure resolve, strong hosting rebuilds, and settlement passes). This is the production solution for Chrome stack moves across mixed-res monitors; combined with robust re-home on metrics changes, home-screen mismatch guard + smart `preferredInitialDockPositionAndSide` on `startTracking` / recurring sync, and always-live reference geometry. Guarantees correct attachment on *any* monitor combination (moves + permanent residence on smaller secondary lower-res displays) without manual "Reset Widget Position". This was the hardest real-world edge; the complete four-layer mechanism (INVESTIGATOR + CODER + TESTER's `docs/testing-multi-display.md`) is detailed in the multi-monitor summary above and makes the attached drawer widget production-ready for open-source release.
